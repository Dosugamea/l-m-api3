    ...
    def Command(self, alt=[], sources=["User", "Group"], permissions=["ALL"], prefix=True, inpart=False, ignoreCase=False, **dkwargs):
        def __wrapper(func):
            ...
            if prefix and ignoreCase:
                cmds = [p+func.__name__.lower().replace("_", " ")
                        for p in self.prefix]
                cmds += [c for in_p in [[p+a.lower().replace("_", " ")
                                         for p in self.prefix] for a in alt] for c in in_p]
            elif prefix:
                cmds = [p+func.__name__.replace("_", " ") for p in self.prefix]
                cmds += [c for in_p in [[p+a.replace("_", " ")
                                         for p in self.prefix] for a in alt] for c in in_p]
            elif ignoreCase:
            ...
            def __check(self, *args, **kwargs):
                chk = args[0].messageCheck(args[1], func.__name__, alt, sources,
                                           permissions, prefix, inpart, ignoreCase, func.cmds, **dkwargs)
                if chk == True:
                    try:
    ...
    def cmd_startup(self):
        ...
        for c in self.mfuncs:
            ...
            if c.prefix and c.ignoreCase:
                self.maybe_cmds.append(
                    [k+c.__name__.lower().replace("_", " ") for k in self.prefix])
            elif c.prefix:
                self.maybe_cmds.append(
                    [k+c.__name__.replace("_", " ") for k in self.prefix])
            elif c.ignoreCase:
            ...
            for a in c.alt:
                if c.prefix and c.ignoreCase:
                    self.maybe_cmds.append(
                        [k+a.lower().replace("_", " ") for k in self.prefix])
                elif c.prefix:
                    self.maybe_cmds.append(
                        [k+a.replace("_", " ") for k in self.prefix])
                elif c.ignoreCase:
    ...
    def userDataCheck(self, msg, **kwargs):
        ...
        for k in kwargs:
            ...
            # a="1"
                ...
                if self.getUser(msg["source"]["userId"], k) != int(kwargs[k]):
                    return False
